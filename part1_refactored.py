...
# Returning the weighted positions of each stock
...
def trade_cycle(e: Exchange, verbose=True):
    ...
    if basket_book and basket_book.bids and basket_book.asks and stock_books[0] and stock_books[0].bids and stock_books[0].asks and stock_books[1] and stock_books[1].bids and stock_books[1].asks:
        ...
        # Checking if profit can be made by selling stocks and if hedge limit is not reached
        ...
        # Checking if profit can be made by selling baskets and if hedge limit is not reached
        if (basket_bid_price > 0.5 * sum(stock_ask_prices)) and abs(max(get_hedge_positions(e))) <= 50:
