...
def trade_cycle(e: Exchange):
    ...
    if basket_book and basket_book.bids and basket_book.asks and stock_books[0] and stock_books[0].bids and stock_books[0].asks and stock_books[1] and stock_books[1].bids and stock_books[1].asks:
        ...
        buyETFvolume = max(basket_bid_volume, min(stock_ask_volume[0], stock_ask_volume[1]), 400)
        sellETFvolume = max(basket_ask_volume, min(stock_bid_volume[0], stock_bid_volume[1]), 400)
        
        if buyETFvolume == 400:
            buyETFvolume = 400 - basket_bid_volume;
            
        if sellETFvolume == 400:
            sellETFvolume = 400 - basket_ask_volume;
        
        buyStockvolume = max(basket_bid_volume, min(stock_ask_volume[0], stock_ask_volume[1]), 400)
        sellStockvolume = max(basket_ask_volume, min(stock_bid_volume[0], stock_bid_volume[1]), 400)
        
        buyStockvolume = int(buyStockvolume/2)
        sellStockvolume = int(sellStockvolume/2)
        
        if buyStockvolume == 200:
            buyStockvolume = 200 - int(basket_bid_volume/2)
            
        if sellStockvolume == 200:
            sellStockvolume = 200 - int(basket_ask_volume/2)
        ...
        if (basket_ask_price < 0.5 * sum(stock_bid_prices)):
            if (basket_bid_price > 0.5 * sum(stock_ask_prices)) and (basket_bid_price - 0.5 * sum(stock_ask_prices) > (0.5 * sum(stock_bid_prices) - basket_ask_price)):
                ...
                bid_response: InsertOrderResponse = e.insert_order(BASKET_INSTRUMENT_ID, price=basket_bid_price, volume=buyETFvolume, side=SIDE_ASK, order_type=ORDER_TYPE_IOC)
                if not bid_response:
                    print("Trade failed")
                print(f"Sold: '{BASKET_INSTRUMENT_ID}' for '{basket_bid_price}'")
                ask_response1: InsertOrderResponse = e.insert_order(STOCK_INSTRUMENT_IDS[0], price=stock_ask_prices[0], volume=buyStockvolume, side=SIDE_BID, order_type=ORDER_TYPE_IOC)
                if not ask_response1:
                    print("Trade failed")
                print(f"Bought: '{STOCK_INSTRUMENT_IDS[0]}' for '{stock_ask_prices[0]}'")
                ask_response2: InsertOrderResponse = e.insert_order(STOCK_INSTRUMENT_IDS[1], price=stock_ask_prices[1], volume=buyStockvolume, side=SIDE_BID, order_type=ORDER_TYPE_IOC)
                if not ask_response2:
                    print("Trade failed")
                print(f"Bought: '{STOCK_INSTRUMENT_IDS[1]}' for '{stock_ask_prices[1]}'")

            else:
                bid_response: InsertOrderResponse = e.insert_order(BASKET_INSTRUMENT_ID, price=basket_ask_price, volume=sellETFvolume, side=SIDE_BID, order_type=ORDER_TYPE_IOC)
                if not bid_response:
                    print("Trade failed")
                print(f"Bought: '{BASKET_INSTRUMENT_ID}' for '{basket_ask_price}'")
                ask_response1: InsertOrderResponse = e.insert_order(STOCK_INSTRUMENT_IDS[0], price=stock_bid_prices[0], volume=sellStockvolume, side=SIDE_ASK, order_type=ORDER_TYPE_IOC)
                if not ask_response1:
                    print("Trade failed")
                print(f"Sold: '{STOCK_INSTRUMENT_IDS[0]}' for '{stock_bid_prices[0]}'")
                ask_response2: InsertOrderResponse = e.insert_order(STOCK_INSTRUMENT_IDS[1], price=stock_bid_prices[1], volume=sellStockvolume, side=SIDE_ASK, order_type=ORDER_TYPE_IOC)
                if not ask_response2:
                    print("Trade failed")
