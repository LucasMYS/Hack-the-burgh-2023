...
def trade_cycle(e: Exchange):
    ...
    if basket_book and basket_book.bids and basket_book.asks:
        ...
        if new_ask_price - new_bid_price > 0.01:
            bid_response: InsertOrderResponse = e.insert_order(BASKET_INSTRUMENT_ID, price=new_bid_price, volume=50, side=SIDE_BID, order_type=ORDER_TYPE_LIMIT)
            print_order_response(bid_response)
            ask_response: InsertOrderResponse = e.insert_order(BASKET_INSTRUMENT_ID, price=new_ask_price, volume=50, side=SIDE_ASK, order_type=ORDER_TYPE_LIMIT)
