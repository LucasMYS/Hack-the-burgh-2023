...
def print_report(e: Exchange, initial_pnl):
    pnl = e.get_pnl()
    ...
    logger.info(f'My delta PNL is: {pnl - initial_pnl:.2f}')
...
def trade_cycle(e: Exchange, initial_pnl):
    ...
    stock_book = [e.get_last_price_book(ID) for ID in STOCK_INSTRUMENT_IDS]
    if basket_book and basket_book.bids and basket_book.asks:
        ...
        new_bid_price = basket_book.bids[0].price + 0.01
        new_ask_price = basket_book.asks[0].price - 0.001
        if new_ask_price - new_bid_price > 0.01:
            bid_response: InsertOrderResponse = e.insert_order(BASKET_INSTRUMENT_ID, price=new_bid_price, volume=20, side=SIDE_BID, order_type=ORDER_TYPE_LIMIT)
            print_order_response(bid_response)
            ask_response: InsertOrderResponse = e.insert_order(BASKET_INSTRUMENT_ID, price=new_ask_price, volume=30, side=SIDE_ASK, order_type=ORDER_TYPE_LIMIT)
...
def main():
    ...
    while True:
        trade_cycle(exchange, initial_pnl)
